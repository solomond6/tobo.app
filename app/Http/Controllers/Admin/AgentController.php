...
class AgentController extends Controller
{
    ...
    public function viewAgent(Request $request, $id=null){
        ...
        return view('admin.agent_details')->with(compact('agentDetails'));
    }


    public function agentProfile(Request $request, $id=null){
        ...
        //var_dump($companyDetails);exit;
        return view('agent.agent_details')->with(compact('agentDetails'));
    }
    ...
    //     return view('admin.set_commission')->with(compact('user'));
    // }
    ...
    public function agentData(){
        $users = DB::table('users')
                    ...
                    ->select('users.*', 'u1.first_name', 'u1.last_name', 'u1.phone_no','user_balance.amount as balance','user_balance.credit as credit', 'u2.first_name as referral_first_name', 'u2.last_name as referral_last_name')
                    ->get();

        return Datatables::of($users)
                ->addColumn('action', function($users){
                    if($users->status == 1){
                        return '<a href="/admin/deactive-agent/'.$users->id.'" class="btn btn-danger btn-sm">De-Activate Agent</a>';
                    }else if ($users->status == 2 || $users->status == 0){
                        return '<a href="/admin/active-agent/'.$users->id.'" class="btn btn-success btn-sm">Activate Agent</a>';
                    }
                })
                ->addColumn('delete-action', function($users){
                    return '<a href="/admin/delete-agent/'.$users->id.'" class="btn btn-danger btn-sm">Delete</a>';
                ...
                ->addColumn('view-details', function($users){
                    ...
                    return '<a href="/admin/view-agent/'.$users->id.'" class="btn btn-info btn-sm">View Detials</a>';
                })
                ->addColumn('name', function($users){
                    return $users->first_name ." ". $users->last_name ;
                })
